...
const applyChartStyles = (chart: Chart, settings: ChartSettings) => chart.setStyles({ grid: { horizontal: { show: true, color: withOpacity(settings.gridColor, settings.gridOpacity), style: settings.gridStyle, size: settings.gridSize, dashedValue: [2, 2] }, vertical: { show: true, color: withOpacity(settings.gridColor, settings.gridOpacity), style: settings.gridStyle, size: settings.gridSize, dashedValue: [2, 2] } }, crosshair: { show: true, horizontal: { show: true, line: { show: true, color: '#94a3b8', style: 'dashed', size: 1, dashedValue: [4, 2] } }, vertical: { show: true, line: { show: true, color: '#94a3b8', style: 'dashed', size: 1, dashedValue: [4, 2] } } }, candle: { tooltip: { showRule: settings.showChartInfo ? 'always' : 'none' } }, indicator: { tooltip: { showRule: settings.showChartInfo ? 'always' : 'none' } }, xAxis: { tickText: { color: settings.textColor } }, yAxis: { tickText: { color: settings.textColor } } })
...
const orderLineLabel = (order: DesktopOrder, position?: DesktopPosition | null, settings?: DesktopTradingSettings | null, sessionCapital = 0) => {
  ...
  const isSl = order.order_type === 'STOPLOSS' || order.is_stoploss
  if (position && position.side !== 'FLAT' && ((position.side === 'LONG' && order.side === 'SELL') || (position.side === 'SHORT' && order.side === 'BUY'))) {
    ...
    if (settings?.desktop_pnl_display_mode === 'percent') {
      ...
      return `${isSl ? 'SL' : order.order_type === 'LIMIT' ? 'L' : 'T'}${isSl ? ` ${qty}` : ''} ${pnl >= 0 ? '+' : ''}${((pnl / base) * 100).toFixed(1)}%`
    }
    return `${isSl ? 'SL' : order.order_type === 'LIMIT' ? 'L' : 'T'}${isSl ? ` ${qty}` : ''} ${pnl >= 0 ? '+' : ''}${Math.round(pnl)}`
  }
...
export function ChartTile({ symbol, interval, supportedIntervals, onIntervalChange, candles = demo, loading, message, settings, isReplaying, isLive, instrument, baseUrl, drawingRequest, onDrawingError, onConfigure, onMaximize, maximized, active, indicators, drawingCommand, drawingAction, drawingMode, onDrawingComplete, onActivate, trades = [], openOrders = [], strategies = [], position = null, sessionCapital = 0, lotSize = 1, tradingSettings = null, tradingEnabled = false, orderEntryEnabled = false, underlyingStrategyEnabled = false, showHistoricalMarkers = false, onToggleHistoricalMarkers, pricePickAction = null, onPricePick, onOrderDrag, onStrategyDrag, onOrderCancel, onOrderConvertRequest, onOrderQuantityUpdate, onChartOrderAction, swapTargets = [] }: { symbol: string; interval: string; supportedIntervals: number[]; onIntervalChange: (interval: string) => void; candles?: Candle[]; loading: boolean; message: string; settings: ChartSettings; isReplaying: boolean; isLive: boolean; replayDatasetKey?: string; instrument: Record<string, unknown>; baseUrl: string; drawingRequest: DrawingRequest; onDrawingError?: (error: unknown) => void; onConfigure: () => void; onMaximize: () => void; maximized: boolean; active: boolean; indicators: string[]; drawingCommand: DrawingCommand | null; drawingAction: DrawingAction | null; drawingMode: DrawingMode; onDrawingComplete: (commandId: number, tool: string) => void; onActivate: () => void; trades?: DesktopTrade[]; openOrders?: DesktopOrder[]; strategies?: DesktopStrategy[]; position?: DesktopPosition | null; sessionCapital?: number; lotSize?: number; tradingSettings?: DesktopTradingSettings | null; tradingEnabled?: boolean; orderEntryEnabled?: boolean; underlyingStrategyEnabled?: boolean; showHistoricalMarkers?: boolean; onToggleHistoricalMarkers?: () => void; pricePickAction?: PricePickAction | null; onPricePick?: (price: number) => void; onOrderDrag?: (order: DesktopOrder, price: number) => void; onStrategyDrag?: (strategyId: string, price: number) => void; onOrderCancel?: (order: DesktopOrder) => void; onOrderConvertRequest?: (order: DesktopOrder, target: ConversionTarget) => void; onOrderQuantityUpdate?: (order: DesktopOrder, quantity: number) => Promise<void>; onChartOrderAction?: (action: OrderAction, price: number, anchor: { x: number; y: number }) => void; swapTargets?: TileSwap[] }) {
  ...
  useEffect(() => {
    ...
    buildTradeMarkers(trades, instrument, intervalSeconds, historicalMarkersVisible).forEach(marker => {
  ...
  }, [trades, instrument, interval, candles, historicalMarkersVisible])
  useEffect(() => {
    if (!isLive) return
    setClock(Date.now())
  ...
  useEffect(() => { setSelectedOrderQty(selectedOrder ? String(selectedOrder.quantity) : '') }, [selectedOrder?.order_id, selectedOrder?.quantity])
  const selectedOrderIsSl = Boolean(selectedOrder && (selectedOrder.is_stoploss || selectedOrder.order_type === 'STOPLOSS'))
  const selectedOrderExitSide = position?.side === 'LONG' ? 'SELL' : position?.side === 'SHORT' ? 'BUY' : null
  const selectedOrderCoveredElsewhere = selectedOrder
    ? openOrders.filter(order => order.order_id !== selectedOrder.order_id && order.status === 'PENDING' && order.side === selectedOrderExitSide && (order.is_stoploss || order.order_type === 'STOPLOSS' || order.order_type === 'LIMIT'))
      .reduce((sum, order) => sum + order.quantity, 0)
    : 0
  const selectedOrderMaxQty = Math.max(0, (position?.quantity ?? 0) - selectedOrderCoveredElsewhere)
  const saveSelectedOrderQty = async () => {
    const quantity = Number(selectedOrderQty)
    if (!selectedOrder || !selectedOrderIsSl || !Number.isInteger(quantity) || quantity < lotSize || quantity > selectedOrderMaxQty || quantity % lotSize !== 0 || !onOrderQuantityUpdate) return
    setSavingOrderQty(true)
    setOrderQtyError('')
    try { await onOrderQuantityUpdate(selectedOrder, quantity) }
    catch (error) { setOrderQtyError(String(error)) }
    finally { setSavingOrderQty(false) }
  }
  ...
  const lineActionPosition = selectedOrderAnchor ? placeNearPoint(selectedOrderAnchor.x, selectedOrderAnchor.y, selectedOrderIsSl ? 620 : 360, selectedOrderIsSl ? 72 : 46) : { left: 12, top: 64 }
  const swapButtons = swapTargets.map(target => <button key={`${target.dir}-${target.label}`} className="icon-button" title={target.label} aria-label={target.label} onClick={event => { event.stopPropagation(); target.onClick() }}>{target.dir}</button>)
  return <section className={`chart ${active ? 'active-chart' : ''}`} style={{ background: settings.background, color: settings.textColor }} onPointerDownCapture={onActivate}><div className="chart-head"><span>{tradingSettings?.desktop_hide_chart_labels ? '' : `${symbol} · `}<select className="interval-picker" value={interval.replace('m', '')} onChange={event => onIntervalChange(event.target.value)} aria-label="Candle interval">{supportedIntervals.map(value => <option key={value} value={value}>{value}m</option>)}</select>{tradingSettings?.desktop_hide_chart_labels ? '' : ' · IST'}</span><span className="chart-actions">{pricePickAction && <span className="candle-close">Pick price</span>}{isLive && <span className="candle-close" aria-label={`Candle closes in ${closeCountdown}`}>{closeCountdown}</span>}{swapButtons}<button className="icon-button" title="Fit data to chart" aria-label="Fit data to chart" onClick={fitChart}>⤧</button><button className="icon-button" title={historicalMarkersVisible ? 'Hide closed trade markers' : 'Show closed trade markers'} aria-label={historicalMarkersVisible ? 'Hide closed trade markers' : 'Show closed trade markers'} onClick={() => { setHistoricalMarkersVisible(value => !value); onToggleHistoricalMarkers?.() }}>◎</button><button className="icon-button" title="Choose instrument" aria-label="Choose instrument" onClick={onConfigure}>⌕</button><button className="icon-button" title={maximized ? 'Restore chart' : 'Maximize chart'} aria-label={maximized ? 'Restore chart' : 'Maximize chart'} onClick={onMaximize}>{maximized ? '⊡' : '⛶'}</button></span></div><div className="kline-container"><div className="kline" ref={element} />{floatingLabels.map(label => <div key={label.key} className="order-line-label" style={{ left: label.x, top: label.y, borderColor: label.color, color: label.color }}>{label.text}</div>)}{pnlLabel && <div className="position-pnl-label" style={{ left: pnlLabel.x, top: pnlLabel.y, color: pnlLabel.color, borderColor: pnlLabel.color }}>{pnlLabel.text}</div>}{selectedOrder && <div className="line-actions" style={{ left: lineActionPosition.left, top: lineActionPosition.top }}><span>{orderLineLabel(selectedOrder, position, tradingSettings, sessionCapital)}</span>{selectedOrderIsSl && <><input aria-label="Stop-loss quantity" type="number" min={lotSize} max={selectedOrderMaxQty} step={lotSize} value={selectedOrderQty} disabled={selectedOrderMaxQty < lotSize || savingOrderQty} onChange={event => { setSelectedOrderQty(event.target.value); setOrderQtyError('') }} onWheel={event => { if (document.activeElement === event.currentTarget) { event.preventDefault(); const current = Number(selectedOrderQty) || selectedOrder.quantity; setSelectedOrderQty(String(Math.max(lotSize, Math.min(selectedOrderMaxQty, current + (event.deltaY < 0 ? lotSize : -lotSize))))) } }} style={{ width: 76 }} /><button disabled={savingOrderQty || !Number.isInteger(Number(selectedOrderQty)) || Number(selectedOrderQty) < lotSize || Number(selectedOrderQty) > selectedOrderMaxQty || Number(selectedOrderQty) % lotSize !== 0} onClick={() => void saveSelectedOrderQty()}>{savingOrderQty ? '…' : 'Save Qty'}</button>{orderQtyError && <span title={orderQtyError} style={{ color: '#f87171' }}>!</span>}</>}<button title="Convert to limit" aria-label="Convert to limit" onClick={() => onOrderConvertRequest?.(selectedOrder, 'LIMIT')}>L</button><button title="Convert to stop-loss" aria-label="Convert to stop-loss" onClick={() => onOrderConvertRequest?.(selectedOrder, 'STOPLOSS')}>SL</button><button title="Convert to target" aria-label="Convert to target" onClick={() => onOrderConvertRequest?.(selectedOrder, 'TARGET')}>T</button><button title="Delete order" aria-label="Delete order" onClick={() => onOrderCancel?.(selectedOrder)}>×</button></div>}{contextMenu && <div className="chart-context-menu" style={{ left: menuPosition.left, top: menuPosition.top }}><strong>{orderEntryEnabled ? `Use as SL ${contextMenu.price.toFixed(2)}` : `Underlying ${contextMenu.price.toFixed(2)}`}</strong>{hasPosition && <><button onClick={() => menuAction('START_AGGRESSIVE_SL')}>Aggressive SL</button><button onClick={() => menuAction('START_BREAKEVEN')}>Breakeven</button></>}{orderEntryEnabled && <><button onClick={() => menuAction('USE_SL_BUY')}>Buy entry</button>{tradingSettings?.context_menu_sl_mode === 'both' && <button onClick={() => menuAction('USE_SL_SELL')}>Sell entry</button>}{hasSlOrders && <button onClick={() => menuAction('BULK_MOVE_SL')}>Move SL here</button>}{hasClosingOrders && <button onClick={() => menuAction('BULK_LIMIT')}>Move exits to Limit here</button>}<button onClick={() => menuAction('START_TARGET_PROFIT')}>Take profit here</button><button onClick={() => menuAction('START_LOCK_PROFIT')}>Lock profit here</button></>}{underlyingStrategyEnabled && <><button onClick={() => menuAction('START_UNDERLYING_TARGET')}>Underlying target here</button><button onClick={() => menuAction('START_UNDERLYING_SL')}>Underlying SL here</button></>}</div>}{loading && <div className="chart-loading"><span className="spinner" />Loading candles...</div>}{!loading && message && <div className="chart-loading chart-message">{message}</div>}</div></section>
}
