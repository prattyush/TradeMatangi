...
export default function App() {
  ...
  const onDrawingComplete = () => { if (drawingMode === 'once') setActiveDrawingTool(null) }
  ...
  const startLive = async () => { try { setLiveError(''); clearLiveTickCache(); setLiveSnapshot(await liveRequest('start', 'POST', { tiles: liveTiles() })) } catch (error) { setLiveError(String(error)) } }
  const stopLive = async () => { if (!live) return; try { await liveRequest(`${live.stream_id}/stop`, 'POST'); setLiveSnapshot(null); clearLiveTickCache() } catch (error) { setLiveError(String(error)) } }
  ...
  const startRun = async () => { try { setReplayError(''); const date = runDate; setScreens(current => current.map(screen => screen.id === activeScreenId ? { ...screen, tiles: screen.tiles.map(tile => ({ ...tile, tradingDate: date })) } : screen)); const tiles = activeScreen.tiles.map(tile => { const item = catalogue.find(entry => entry.symbol === tile.symbol) ?? fallbackCatalogue[0]; const instrument = tile.kind === 'option' ? { kind: 'option', exchange: item.exchange, underlying: tile.symbol, expiry: tile.expiry, strike: Number(tile.strike), right: tile.right } : { kind: item.chart_type ?? 'equity', exchange: item.exchange, symbol: tile.symbol }; return { tile_id: tile.id, instrument } }); const backendInterval = mode === 'Stepwise' ? Math.min(...activeScreen.tiles.map(tile => Number(tile.interval))) : Number(activeScreen.tiles[0].interval); const next = await replayRequest('start', 'POST', { mode: mode.toLowerCase(), date, start_time: `${runStartTime}:00`, interval_seconds: backendInterval * 60, speed: Number(replaySpeed), tiles }); setReplay(next) } catch (error) { setReplayError(String(error)) } }
  ...
  return <main>
    <header>
      ...
      {(mode === 'Replay' || mode === 'Stepwise') && <span className="run-controls"><label>Date <input type="date" value={runDate} onChange={event => setRunDate(event.target.value)} disabled={Boolean(replay && replay.state !== 'stopped')} /></label><label>Start <input type="time" value={runStartTime} onChange={event => setRunStartTime(event.target.value)} disabled={Boolean(replay && replay.state !== 'stopped')} step="60" /></label>{mode === 'Replay' && <label>Speed <select value={replaySpeed} onChange={event => setReplaySpeed(event.target.value)} disabled={Boolean(replay && replay.state !== 'stopped')}><option value="0.25">0.25×</option><option value="0.5">0.5×</option><option value="1">1×</option><option value="1.1">1.1×</option><option value="1.25">1.25×</option><option value="1.5">1.5×</option><option value="2">2×</option><option value="5">5×</option><option value="10">10×</option></select></label>}{!replay || replay.state === 'stopped' ? <button onClick={startRun}>Start</button> : <>{mode === 'Replay' && <button onClick={() => replayAction(replay.state === 'paused' ? 'resume' : 'pause')}>{replay.state === 'paused' ? 'Resume' : 'Pause'}</button>}{mode === 'Stepwise' && <button onClick={() => replayAction('next-bar')}>Next bar</button>}<button onClick={() => replayAction('stop')}>Stop</button><small>{replay.bar_index} · {new Date(replay.cursor * 1000).toISOString().slice(11, 19)}</small></>}</span>}
      ...
      <button className="icon-button" title="Chart settings" aria-label="Chart settings" onClick={() => setShowSettings(true)}>⚙</button><span className={`connection ${connection}`}>● {connection}</span><button onClick={() => { if (hasNativeHost) void invoke('desktop_logout'); setBrowserToken(''); setLiveSnapshot(null); clearLiveTickCache(); setConnection('authentication_required') }}>Log out</button>
    </header>
