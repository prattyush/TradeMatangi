...
export default function App() {
  ...
  const login = async () => { try { setLoginError(''); await invoke('desktop_login', { baseUrl: serverUrl, email, password }); setConnection('connected') } catch (error) { setConnection('authentication_required'); setLoginError(String(error)) } }
  const logout = async () => { await invoke('desktop_logout'); setConnection('authentication_required') }
