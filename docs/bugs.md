...
**[RESOLVED]** Breeze BFO option ticks (SENSEX) lose right/strike identity in `_on_ticks` — candle accumulators use raw token id and right tag never resolved (BUG-XIII-2).
- **Symptom**: Starting a paper or real trading session for `BSESEN` (SENSEX) options on Breeze subscribes successfully and option ticks DO arrive on the WebSocket, but the production tick handler classifies every BFO option tick as if it were an equity tick — the candle accumulator key ends up `8.1!855562_EQ` instead of `8.1!855562_CE` / `8.1!855562_PE`. CE and PE candles never separate, the candle payload pushed to `session.paper_tick_queue` has no `right` field, and downstream consumers (UI, PnL, analytics) silently drop or misattribute the ticks.
- **Root cause**: Three independent problems in `BreezeStreamManager._on_ticks` for BFO option payloads.
  1. **BFO option ticks omit `right`/`strike_price`.** Production code's `_on_ticks` reads `tick.get("right", "")` and applies a `_right_map` lookup; for BFO, Breeze's WS payload has neither field (raw payload example: `{'symbol': '8.1!855562', ..., 'OI': 2376380, 'CHNGOI': 2007680, 'quotes': 'Quotes Data'}`). The `_right_map.get(right_raw)` falls back to `None`, so the candle is emitted as an equity tick.
  2. **`get_quotes()` is unreliable for BFO.** The first attempted fix used `breeze.get_quotes(stock_code, exchange_code, expiry, product_type='options', right, strike)` at subscribe time to discover the ScripCode→(right, strike) mapping. For NFO this works; for BFO it raises `Expecting value: line 1 column 1 (char 0)` (empty/non-JSON response from Breeze), so the map stays empty and the lookup falls through to the equity path.
  3. **`quotes: "Quotes Data"` is a misleading discriminator.** Both Breeze index and BFO option ticks carry `quotes: "Quotes Data"`. Using that field as an option-tick marker misclassifies SENSEX/NIFTY index ticks as option ticks, breaking the index detection in `scripts/test_broker_streaming.py` (subscription to CE/PE never fires).
- **Fix**: Replace the `get_quotes()` round-trip with a parser for the Breeze Security Master file, and use `OI`/`CHNGOI` (not `quotes`) as the option-tick discriminator.
  1. **`backend/app/services/breeze_master.py`** (new): shared module exposing `ensure_security_master_downloaded(exchange_code)` and `load_breeze_security_master(stock_code, exchange_code, strike, right, expiry_breeze)`. Downloads `https://directlink.icicidirect.com/NewSecurityMaster/SecurityMaster.zip` once per day (Breeze regenerates at 8 AM) and caches under `<DATA_DIR>/ICICISecurityMaster/` (uses local `FOBSEScripMaster.txt` / `FONSEScripMaster.txt` if present). Returns `{ScripCode: (strike, right)}` for the requested instrument.
  2. **`backend/app/services/breeze_service.py`**:
     - Added `self._option_scrip_map: dict[str, tuple[int, str]] = {}` populated at subscribe time via `_build_scrip_map()` → `breeze_master.load_breeze_security_master()`. Each subscribed option's ScripCode (e.g. `855562` for SENSEX PE 77100) is resolved from the master CSV.
     - `_on_ticks` now detects option ticks by `"OI" in tick or "CHNGOI" in tick` (open interest — option-only). For BFO ticks that lack `right`, it parses `tick["symbol"]` (e.g. `8.1!855562`), extracts the ScripCode, and looks up `(strike, right)` from the map. The candle key then becomes `f"{name}_{right or 'EQ'}"` = `8.1!855562_PE` — distinct from `8.1!855562_CE`.
  3. **`scripts/test_broker_streaming.py`**:
     - Imports `breeze_master.load_breeze_security_master` to build the ScripCode map at subscribe time.
     - Fixed index-tick detection: `is_option_tick = "OI" in tick or "CHNGOI" in tick` (no longer relies on `quotes == "Quotes Data"` which collides with index ticks).
     - Cleaned up log line — dropped the full raw tick dump from `_on_ticks`, now shows `BREEZE TICK #N: raw_symbol=... exchange=... ltp=... right=PE strike=77100 option=True` only.
